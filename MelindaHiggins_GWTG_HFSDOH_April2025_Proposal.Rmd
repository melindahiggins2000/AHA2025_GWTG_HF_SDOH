...
Time-varying effect models (TVEM) extend the traditional linear models by examining (modeling) how associations between variables change across time (or over a 3rd continuous "moderating" variable with a range of values, such as social-determinant of health (SDOH) risk metrics). `r citep("10.1007/978-3-030-70944-0")` The estimated regression coefficients from TVEM models are estimated such that the nature of change over the 3rd "moderating" variable can be complex and nonlinear. In fact, the nature of this relationship does not need to be specified ahead of time (e.g., determining the functional form such as linear, polynomial or other). Since the functional forms of these relationships can be complex and non-linear, figures are key results from these models which are used to graphically summarize the value of the model coefficients which vary over the "varying-moderator" over continuous time. TVEM model results and figures are then used to identifying values of the varying-moderator at which individuals are at increased risk for adverse outcomes and values over which risk factors are most strongly associated with those outcomes, TVEM can inform where to focus targeted interventions.
...
### Novel Research Questions Possible with TVEM
...
1. How does the association between co-morbidities and risk of death during hospitalization or discharge to hospice vary based on neighborhood level access to healthcare (hospital beds per capita)? Do these varying associations significantly differ by race?
2. How does the prevalence of SGLT2 treatment prescription vary over median income linked to patient zip-code?
3. How does the association between clinical risk factors (such as BMI) and 28-day mortality vary based on environmental exposure to ozone levels? Do these varying associations significantly differ by biological sex?
...
Each aim will incorporate five key SDOH domains [1] Environmental Exposures, [2] Healthcare Access, [3] Community Health Risk, [4] Built Environment/Stress, and [5] Structural Risk Factors, as effect modifiers using time-varying effects models (TVEM) which allow the moderating effects to vary over the range of SDOH metrics as opposed to fixed moderation effects modeled using traditional regression models.

* Specific Aim 1: To assess to what extent SDOH modify and over what SDOH levels the association between predisposing factors and discharge outcomes (death or hospice versus other dispositions) are highest following HFpEF hospitalization.

* Specific Aim 2: To evaluate to what extent SDOH modify and over what SDOH levels the association between predisposing factors and the likelihood of receiving an SGLT-2 (Sodium-glucose Cotransporter-2) inhibitor prescription at discharge is highest among HFpEF patients.

* Specific Aim 3: To determine to what extent SDOH modify and over what SDOH levels the association between predisposing factors and clinical risk burden, as measured by the modified EFFECT (Enhanced Feedback for Effective Cardiac) risk score for both 28-day and 1-year mortality, is strongest in patients hospitalized with HFpEF.
...
Prior to analysis we also will exclude patients who:

* were transferred from hospice at admission [`admitsource == 14`]
* had an LVAD (left-ventricular assistance device) [`medhist_28 == 1`] or 
* had a heart transplant [`medhist_42 == 1`]

_NOTE: After applying these added exclusion critera, there were only another n=1185 patients to be removed._
...
* OUTCOME 1: Discharge Status - either death [`dschstati == 6`] or discharged to hospice either at home [`dschstati == 2`] or to a healthcare facility [`dschstati == 2`]
...
* OUTCOME 2: Prescribed SGLT2 [`sglt2i_disc == 1`] _NOTE: SGLT2 data is only available from 2019-2022._
...
* OUTCOME 3: modified-EFFECT score from ARIC Trial will include these variables to compute 28-day mortality and 1-year mortality risk `r citep("10.1161/CIRCHEARTFAILURE.117.003992")`
    ...
    - hypoxia - this variable was not captured in the GWTG-HF database, but we will used these GWTG-HF database variables to create a composite indicator of low-oxygen risk:
        - `medhist_30` (sleep disordered breathing)
        - `sleepequip_01` (use of O2 at home)
...
* Patient Demographics:
    - `agei` (age in year) 
        - _not included for aim 3 since age is already included in the modified-EFFECT score_
    - `genderi` (gender)
    - `racei` and `race2i` (race-ethnicity)
        _not included for aim 3 since race is already included in the modified-EFFECT score_
    - `dmhx` (diabetes)
...
* Clinical Risk Factors:
    ...
    - `symptomsadm_xx` (symptoms)
    - `medhist_17` (renal insufficiency)  
    - `medhist_16` (on dialysis)
    - `medhist_41` (kidney transplant)
    - `egfri` (eGFR for kidney function and stage of kidney disease)
    - `medhist_01` (atrial fibrillation)
    - `medhist_02` (atrial flutter)
    - `activeinfec_xx` (active infections at admission)
    - [for admissions during or post-COVID] we will also consider:
    ...
    - vaccinations
        - `fluvacc` (flu vaccine)
        - `covidvacc` (covid vaccine)
        - `pneumovacc` (pneumococcal vaccine)

#### **Variables for Varying SDOH Moderators for the TVEM Models**
...
* Environmental Exposures
    - Institute for Health Metrics and Evaluation (IHME) data including environmental pollution measures of ozone, nitrous dioxide, PM2.5 particulates
        - These variables are all named `IHME_ZIP_pollutant_mean_year` for each pollutant exposure and year
    - EPA AQS (Environmental Protection Agency - Air Quality System) for data linked by year and patient-level zip code for ozone, PM2.5 particulates, and days with excessive heat
        - These variables are all named `EPH_ZIP_pollutant_metric_AQS_year` for each pollutant and exposure metric by year
...
* Healthcare Access
    - ACS (American Community Survey) measures of hospital bed per capita assessed at the county-level linked by patient zip code and percentage of homes with no Internet access
        - These variables are named `ACS_ZIP_metric_year` for number or percentage of hospital beds and rates of Internet access by year
* Community Health Risk
    - Data from the CDC (Centers of Disease Control and Prevention)'s [PLACES database](https://www.cdc.gov/places/) which provides  ()[https://www.cdc.gov/places/] from health-related data for all U.S. counties, incorporated and census designated places, census tracts, and ZIP Code Tabulation Areas (ZCTAs). These measures include per capita percentage for: teeth lost among adults aged 65 years or older (per capita), adults aged 18 years or older reporting no leisure-time physical activity, asthma prevalence, and rates of depression.
        - These variables are named `PLACES_ZIP_metric_year` for each health metric and year
...
* Built Environmental Exposures
    - [NANDA (National Neighborhood Data Archive)](https://nanda.isr.umich.edu/data/) which was "created to facilitate research on the relationship between neighborhoods and health". These variables include: number of parks and daily traffic counts.
        - These variables are named `NANDA_ZIP_metric_year` for each reported metric and year
* Structural Risk Factors
    - IHME data including county-specific measures (like median income) as well as race/ethnicity specific measures (like life expectancy and levels of home ownership) and race + age + sex specific measures (like mortality all cause and years of life lost)
        - These variables are named `IHME_ZIP_metric` for each metric reported

### Missing Data

A missing data assessment will be performed for all variables to be included for each analysis model. Patients that are missing any of the outcome variables, key predictors or SDOH metrics will be removed from analysis. Where possible, a comparison tests (t-tests, chi-square tests and non-parametric equivalent tests) will be performed to determine if there are any significant differences between for patients retained or removed from the final analyses. For the patients retained for analysis, any remaining missing data for covariates will be imputed using the multiple imputations for multivariate missing data procedures implemented in the `mice` R package.`r citep("10.18637/jss.v045.i03")`


## Example TVEM Analyses - Preliminary findings and demonstration of the proposed analysis workflow
...
This TVEM model estimates show that in general the prevalence of death or discharge to hospice range from approximately 2% to around 6-7%. The prevalence does vary but reaches local maxima around $65K-$70K median income with a slight reduction for higher incomes of $90K-$100K and then rises again around $115K-$120K which illustrates that the effect of the SDOH of median income is not constant.
...
As can be seen the prevalence of SGLT2 prescription is very low (< 0.5%). While the levels are low, the prevalence does increase steadily from lower to higher median income levels.
...
Similar to the overall prevalence shown above, the prevalence of SGLT2 prescription is very low (< 0.5%) but does show that male patients are more likely to get SGLT2 prescriptions compared to female patients - and these difference increase steadily from lower to higher median income levels. Interestingly, female patients living in higher income areas have some of the lowest SGLT2 prescription rates.

![Figure: Model 2c](tvem_sglt2_byincome_bysex.png) 

## Anticipated Issues and Potential Limitations

Anticipated Issues:

* Missing data due to changing data collection methods and treatment guidelines over the last 20 years (from 2005-2022).
* Low proportions of SGLT-2 prescriptions after beginning in 2019.
* There is no direct measure of hypoxia in the database, so we plan to use a composite of other measures as a proxy for low-oxygen status or risk (i.e., sleep disordered breathing, use of oxygen at home, ECMO (extracorporeal membrane oxygenation), mechanical ventilation).

Potential Limitations: 

* The outcomes to be investigated in this study will be limited to only those documented during hospital stays which were compiled in the AHA GWTG-HF database.
* SGLT2 as a recommended treatment for HFpEF is a recent recommendation, as such SGLT2 prescriptions are only reported from 2019-2022, so aim 2 will be limited to these 4 years.
* Our calculation of the modified-EFFECT score will not be computed exactly due to only proxy measures of hypoxia available in the database.
...
knitr::write_bib(c(.packages(), "tvem", "mice", "ggplot2", "dplyr"), "packages.bib")
```
