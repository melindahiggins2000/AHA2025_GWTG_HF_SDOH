...
For the TVEM models, we will consider the social determinants of health (SDOH) which are already linked at the patient-level by patient address zipcodes. These variables cover the following SDOH domains:
...
* Environmental Exposures
    ...
    - EPA AQS (Enrionmental Protection Agency - Air Quality System) for data linked by year and patient-level zipcode for ozone, PM2.5 particulates, and days with excessive heat
* Healthcare Access
    - ACS (American Community Survey) measures of hospital bed per capita assessed at the county-level linked by patient zipcode and percentage of homes with no Internet access
...
## Preliminary Findings - Example Analysis 

The current GWTG Full HF Dataset has 2,567,080 cases with the following breakdown by HF Type:

| HF Type           | Number Cases  | Percent  |
|:------------------|--------------:|---------:|
| HFrEF (EF <= 40)  | 1110467       |  43.26 % |
| HFmrEF (EF 41-49) |  220347       |   8.58 % |
| HFpEF (EF >= 50)  |  991545       |  38.63 % |
| LVEF missing      |  244721       |   9.53 % |
